...
#load the dataset
    ...
    filter_frac = data.make_Y_ttbar(data['j2_features'], tau32_cut = options.tau32_cut, deepcsv_cut = options.deepcsv_cut, mlow=options.mlow, mhigh=options.mhigh)
    if(options.randsort):
        data2.make_Y_ttbar(data2['j1_features'], tau32_cut = options.tau32_cut, deepcsv_cut = options.deepcsv_cut, mlow=options.mlow, mhigh=options.mhigh, extra_str = '2')



    if(do_val):
        val_data.make_Y_ttbar(val_data['j2_features'], tau32_cut = options.tau32_cut, deepcsv_cut = options.deepcsv_cut, mlow=options.mlow, mhigh=options.mhigh)
        if(options.randsort):
            val_data2.make_Y_ttbar(val_data2['j1_features'], tau32_cut = options.tau32_cut, deepcsv_cut = options.deepcsv_cut, mlow=options.mlow, mhigh=options.mhigh, extra_str = '2')
    ...
    for model_idx in range(options.num_models):
        ...
        if(options.use_images):
            ...
            if(options.small_net):
                ...
                model = dense_med_net(dense_shape)
...
if(__name__ == "__main__"):
    ...
    parser.add_argument("--mlow", default = 105., type = float, help = "What tau32 cut to use on tag side")
    parser.add_argument("--mhigh", default = 220., type = float, help = "What tau32 cut to use on tag side")
