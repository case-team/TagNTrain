...
def draw_sys_variations(options, frac_const = 5):
    ...
    feature_names = ["jet mass", r'$\tau_1$', r"$\tau_{21}$", r"$\tau_{32}$", r"$\tau_{43}$", "LSF3", "DeepB", "nPFCands", "mjj"]
    flabels = ["jetmass","tau1", "tau21", "tau32", "tau43", "LSF3", "DeepB", "nPFCands", "mjj"]
    ...
    j1_m_nom =   sig_only_data["j1_features"][:,0]
    m_max = np.percentile(j1_m_nom, 100. - cutoff)
    m_min = np.percentile(j1_m_nom, cutoff)
    ...
    for jme_var in JME_vars_clean:
        ...
        j1_m_up =   np.clip(sig_only_data["j1_JME_vars"][:, JME_vars_map["m_" + up_str]], m_min, m_max)
        j1_m_down = np.clip(sig_only_data["j1_JME_vars"][:, JME_vars_map["m_" + down_str]], m_min, m_max)



        ns, bins, ratios, frac_unc = make_multi_ratio_histogram([j1_m_nom, j1_m_up, j1_m_down], labels, 
    ...
    for sys in sys_weight_list_clean:
        ...
        for i in range(len(feature_names)):
            ...
            if(feat_name == 'mjj'): feat = mjj
                ...
                feat = sig_only_data["j1_features"][:,i]
            ...
            ns, bins, ratios, frac_unc = make_multi_ratio_histogram([feat, feat, feat], labels, 
                ...
                normalize=norm, save = True, fname=options.output + sys + '_' + flabels[i]  + ".png", unc_band_norm = num_sig_tot)
